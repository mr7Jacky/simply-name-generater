...
class RNN(nn.Module):
    ...
    * There’s also a dropout layer, which randomly zeros parts of its input with a given probability (here 0.1)
    ...
    def forward(self, category, input, hidden):
        input_combined = torch.cat((category, input, hidden), 1).to(self.device)
        hidden = self.i2h(input_combined).to(self.device)
        output = self.i2o(input_combined).to(self.device)
